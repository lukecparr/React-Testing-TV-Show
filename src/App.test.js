...
test('Renders App without error', () => {
	mockFetchShow.mockResolvedValueOnce({data: results});
	render(<App />);

	expect(() => screen.getByText('Fetching data...')).not.toThrow();
	
});

test('App calls fetchShow and gets data in return', async () => {
	mockFetchShow.mockResolvedValueOnce({data: results});
	render(<App />);

	expect(mockFetchShow).toBeCalled();

	const poster = await waitFor(() => screen.getAllByTestId('poster'));
